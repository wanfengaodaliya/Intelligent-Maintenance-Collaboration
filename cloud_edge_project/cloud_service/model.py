"""Mock cloud bearing model for cloud-route packets."""
...
from common.schemas import require_confidence, validate_cloud_request
...
def infer_cloud(request: dict[str, Any]) -> dict[str, Any]:
    start = perf_counter()
    validated = validate_cloud_request(request)
    packet = validated["packet"]
    edge_result = validated["edge_result"]
    edge_confidence = require_confidence(edge_result["confidence"], "edge_result.confidence", packet["packet_id"])
