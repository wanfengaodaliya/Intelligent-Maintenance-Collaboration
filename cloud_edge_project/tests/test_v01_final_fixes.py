...
def test_edge_scheduler_post_uses_configured_service_url(monkeypatch):
    ...
    def post(url, **kwargs):
        ...
        return SimpleNamespace(
            ...
            json=lambda: {"route": "edge"},
        )

    monkeypatch.setattr(edge_app, "config", {"services": {"scheduler": {"host": "10.9.8.7", "port": 9123}}})
    monkeypatch.setattr(edge_app.requests, "post", post)

    edge_app._post_scheduler_packet_route("/scheduler/decide", {})

    assert observed["url"] == "http://10.9.8.7:9123/scheduler/decide"
