...
def build_sensor_packet(
    *,
    ...
    run_id: str | None | object = _RUN_ID_NOT_PROVIDED,
) -> dict[str, Any]:
    ...
    if run_id is not _RUN_ID_NOT_PROVIDED:
        if not isinstance(run_id, str) or not run_id.strip():
            raise PacketValidationError("run_id must be a non-empty string when provided")
        if len(run_id) > 128:
