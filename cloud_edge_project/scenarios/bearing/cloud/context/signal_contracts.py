"""Bearing signal / operating-context contracts and their validators.

This is the real implementation of the bearing-specific sensor-packet, edge
feature-summary and legacy envelope validators originally living in
``common.schemas``. ``common.schemas`` keeps a compatibility re-export so the
old public import paths and JSON/error contracts stay unchanged.

The generic primitives (``require_*``, ``ContractError``, ``LABELS``,
``RISK_LEVELS``, ``ROUTES``) are pulled from ``common.schemas``.
"""
