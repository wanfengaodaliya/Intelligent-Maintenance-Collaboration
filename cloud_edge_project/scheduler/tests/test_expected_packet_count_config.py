...
from compatibility.bearing_v12.scheduler_mapper import assignment_to_legacy
...
def test_repository_persists_run_id_for_peer_edge_exclusion(tmp_path) -> None:
    ...
    legacy_request = assignment_to_legacy(request)
    claim_id = "claim_01"
    claimed = repository.claim(legacy_request, claim_id)
    attempt_id, _ = repository.start_attempt(
        request["task_id"],
        "edge_01",
        claim_id,
        bearing_id=legacy_request["bearing_id"],
    )
...
def test_scheduler_accepts_derived_run_id(monkeypatch) -> None:
    monkeypatch.setenv("SCHEDULER_EXPECTED_PACKET_COUNT", "80")
    request = _request(80)
    request["run_id"] = build_run_id(
        device_id=request["device_id"],
        batch_created_timestamp_ns=request["created_timestamp_ns"],
    )

    assert validate_assignment_request(request)["run_id"] == request["run_id"]
