        ...
        comparisons.append(
            {
                ...
                # Legacy observational field: edge action_grade gap.
                "grade_gap": abs(
...
def _max_grade_gap(comparisons: Sequence[Mapping[str, Any]]) -> int:
    return max(
        (int(comparison["grade_gap"]) for comparison in comparisons), default=0
    )
...
def _conservative_final_state(
    source_results: Sequence[Mapping[str, Any]],
) -> str | None:
    if not source_results:
        return None
    if any(str(result["bearing_state"]) == "fault" for result in source_results):
        return "fault"
    return "normal"
...
def build_window_result(
    ...
    *,
    ...
    # Final action is only produced for a settled, non-conflicted window.
    if sufficient_edge_diversity and not has_conflict:
        final_action_level = max_observed_action_level
        final_action_grade = ACTION_LEVEL_TO_LEGACY_GRADE[final_action_level]
        recommended_action = ACTION_LEVEL_TO_ACTION[final_action_level]
        final_state = _conservative_final_state(source_results)
    else:
    ...
    return {
        ...
        "max_grade_gap": _max_grade_gap(comparisons),
