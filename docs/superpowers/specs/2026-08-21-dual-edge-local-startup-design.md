...
- Summary 建议 LLM 服务固定为 `8005`，仅由宿主机 Summary 调用；两个 Edge 不包含建议 LLM 客户端或配置。云端模型更新 LLM（llama.cpp 3B）固定为 `6006`。启动脚本传 `-SkipLLM` 时同时跳过两者，并让 Summary 使用固定中文模板。
...
- 启动脚本按四个阶段执行，每阶段带健康门（每 2 秒轮询、默认 180 秒超时）：
  ...
  2. 宿主机 Scheduler `8003`、Cloud `8004`（`moment_light_adapt` 后端且模型已加载）与 Summary `8006` HTTP ready；
  3. 两个 LLM `/v1/models` ready（Summary 建议 `8005` + 云端模型更新 `6006`；`-SkipLLM` 时 Summary 使用模板）；
...
- 正式 Compose 的 Edge 拓扑地址（MQTT、Scheduler、Cloud）固定取值，不接受 `.env` 覆盖；其他拓扑只能通过独立 Compose override 文件实现。
...
4. 未传 `-SkipLLM` 时，Summary 建议 LLM 在 `8005`、云端模型更新 LLM 在 `6006` 可用；两个 Edge 均无建议 LLM 配置。传 `-SkipLLM` 时 Summary 使用固定模板。
